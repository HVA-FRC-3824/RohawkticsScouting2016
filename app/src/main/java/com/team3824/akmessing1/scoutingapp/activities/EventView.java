...
public class EventView extends Activity implements AdapterView.OnItemSelectedListener {
    ...
    public void onItemSelected(AdapterView<?> parent, View view, int pos, long id) {
        ...
        switch (pos) {
            ...
            case 13: // Driver Ability
                qualitative(cursor, Constants.DRIVER_ABILITY_RANKING);
                break;
            case 14: // Defense Ability
                qualitative(cursor, Constants.DEFENSE_ABILITY_RANKING);
                break;
            case 15: // Auto
                auto(cursor);
    ...
    private void defenses(Cursor cursor) {
        ...
        Collections.sort(teams, new Comparator<ELI_Defenses>() {
            ...
            public int compare(ELI_Defenses lhs, ELI_Defenses rhs) {
                ...
                float leftValue = (lhs.seens[Constants.LOW_BAR_INDEX] == 0) ? 0 : (lhs.totalCrosses / (float) lhs.seens[Constants.LOW_BAR_INDEX]);
                float rightValue = (rhs.seens[Constants.LOW_BAR_INDEX] == 0) ? 0 : (rhs.totalCrosses / (float) rhs.seens[Constants.LOW_BAR_INDEX]);
        ...
        Collections.sort(teams, new Comparator<ELI_Auto>() {
            ...
            public int compare(ELI_Auto lhs, ELI_Auto rhs) {
                if (lhs.mDefenses.seens[Constants.LOW_BAR_INDEX] == 0 && rhs.mDefenses.seens[Constants.LOW_BAR_INDEX] == 0) {
                    ...
                    float leftValue = (lhs.mDefenses.totalCrosses * 10.0f + lhs.mHigh.mAutoMade * 10.0f + lhs.mLow.mAutoMade * 5.0f) / (float) lhs.mDefenses.seens[Constants.LOW_BAR_INDEX];
                    float rightValue = (rhs.mDefenses.totalCrosses * 10.0f + rhs.mHigh.mAutoMade * 10.0f + rhs.mLow.mAutoMade * 5.0f) / (float) rhs.mDefenses.seens[Constants.LOW_BAR_INDEX];
                    return Float.compare(rightValue,leftValue);
                }
            }
        });
     ...
     *
     * @param cursor The response from the stats database table
     * @param ability Which ability to sort by
     */
    private void qualitative(Cursor cursor, String ability) {
